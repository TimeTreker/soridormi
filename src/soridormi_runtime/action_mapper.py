...
class ActionMapperConfig:
    ...
    The first Open Duck ONNX integration follows the inference convention:

        motor_target = default_pose + action_scale * action

    MuJoCoBackend still clips controls to ctrlrange. This mapper also supports
    optional limits from config when actuator entries include ctrlrange.
    """
...
class PolicyActionMapper:
    ...
    def from_robot_config(
        ...
        return cls(
            ActionMapperConfig(
                ...
                action_scale=float(action_mapping.get("action_scale", DEFAULT_ACTION_SCALE)),
    ...
    def action_to_targets(self, action: np.ndarray | list[float]) -> dict[str, float]:
        ...
        for i, name in enumerate(self.config.joint_names):
            ...
            target = default + self.config.action_scale * float(action_arr[i])
    ...
    def action_to_command(
        ...
        targets_by_name = self.action_to_targets(action)
