...
from soridormi_runtime.clearance_readiness import (
    DEFAULT_REQUIRED_SCENARIOS,
    _as_float,
    _format_value,
    _mapping,
    build_clearance_readiness,
)
...
def build_clearance_candidate_history(
    *,
    ...
    for profile, suite_dir in _discover_suite_dirs(root, profiles):
        readiness = build_clearance_readiness(
            profile=profile,
            suite_dir=suite_dir,
...
def render_markdown(report: M10ClearanceHistoryReport) -> str:
    lines = [
        "# Soridormi clearance qualification clearance candidate history",
        "",
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Summarize clearance qualification clearance candidate history.")
...
def main(argv: list[str] | None = None) -> int:
    ...
    report = build_clearance_candidate_history(
