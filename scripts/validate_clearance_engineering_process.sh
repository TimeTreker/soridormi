...
suite_dir=""
output_dir="${SORIDORMI_CLEARANCE_PROCESS_OUTPUT_DIR:-/tmp/soridormi_clearance_engineering_process}"
run_pytest="1"

usage() {
  cat <<'USAGE'
Usage: ./scripts/validate_clearance_engineering_process.sh [options]

Validate the offline clearance qualification engineering process: candidate-history reporting,
...
echo "Soridormi clearance qualification engineering-process validation"
echo "============================================"
...
echo "Compiling clearance qualification process modules..."
python -m compileall -q \
  src/soridormi_runtime/clearance_candidate_history.py \
  src/soridormi_runtime/clearance_readiness.py \
  src/soridormi_runtime/clearance_visual_inspection.py \
  src/soridormi_runtime/clearance_evidence_package.py \
  src/soridormi_runtime/clearance_teacher_comparison.py
...
echo "Checking clearance qualification process docs..."
rg -n "report_clearance_candidate_history.sh" \
  docs/README.md \
  docs/SORIDORMI_EXECUTION_ROADMAP.md \
  docs/SORIDORMI_TARGET_AND_ROADMAP.md \
  docs/SORIDORMI_WBC_CLEARANCE_CONTROL.md >/dev/null
rg -n "validate_clearance_engineering_process.sh" \
  docs/README.md \
  docs/SORIDORMI_EXECUTION_ROADMAP.md \
  docs/SORIDORMI_TARGET_AND_ROADMAP.md \
...
if [ "${run_pytest}" = "1" ]; then
  echo "Running focused clearance qualification process tests..."
  pytest -q \
    tests/test_clearance_engineering_process.py \
    tests/test_clearance_candidate_history.py \
    tests/test_clearance_readiness.py \
    tests/test_clearance_promotion_gate.py \
    tests/test_clearance_visual_inspection.py \
    tests/test_clearance_evidence_package.py \
    tests/test_clearance_teacher_comparison.py
else
  echo "Skipping focused pytest gate (--skip-pytest)."
fi

echo
echo "clearance qualification engineering-process validation: PASS"
