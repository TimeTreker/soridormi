...
def test_clearance_engineering_process_script_parse_and_stays_offline() -> None:
    repo = Path(__file__).resolve().parents[1]
    script = repo / "scripts" / "validate_clearance_engineering_process.sh"
    ...
    assert "tests/test_clearance_candidate_history.py" in source
    assert "run_sim_server.sh" not in source
    assert "train_clearance_residual_policy.sh" not in source


def test_clearance_engineering_process_script_smoke_without_pytest(tmp_path: Path) -> None:
    repo = Path(__file__).resolve().parents[1]
    ...
    assert "clearance qualification engineering-process validation: PASS" in result.stdout
