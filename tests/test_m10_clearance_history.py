...
from soridormi_runtime.m10_clearance_history import (
    DEFAULT_REFERENCE_PROFILE,
    build_m10_clearance_history,
    render_markdown,
)
...
def test_clearance_history_keeps_reference_when_candidates_regress(tmp_path: Path) -> None:
    ...
    report = build_m10_clearance_history(scenario_eval_root=root)
...
def test_clearance_history_module_cli_strict_fails_without_ready_candidate(tmp_path: Path) -> None:
    ...
    result = subprocess.run(
        [
            sys.executable,
            "-m",
            "soridormi_runtime.m10_clearance_history",
            "--scenario-eval-root",
            str(root),
            "--output-dir",
            str(output_dir),
