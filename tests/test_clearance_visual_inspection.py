...
from soridormi_runtime.clearance_visual_inspection import (
    build_clearance_visual_inspection_plan,
    render_markdown,
)
...
def test_visual_inspection_plan_includes_follow_camera_and_required_scenarios(tmp_path: Path) -> None:
    ...
    plan = build_clearance_visual_inspection_plan(
        profile="candidate",
...
def test_clearance_visual_inspection_cli_functionally_writes_plan_artifacts(tmp_path: Path) -> None:
    readiness_path = tmp_path / "readiness" / "clearance_readiness.json"
    ...
    result = subprocess.run(
        [
            sys.executable,
            "-m",
            "soridormi_runtime.clearance_visual_inspection",
            "--profile-name",
            "candidate",
            "--output-dir",
            str(output_dir),
...
def test_clearance_visual_inspection_script_functionally_blocks_failed_readiness(tmp_path: Path) -> None:
    readiness_path = tmp_path / "readiness" / "clearance_readiness.json"
