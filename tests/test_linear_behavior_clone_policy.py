...
def _write_command_context_linear_model(path: Path) -> None:
    ...
    np.savez(
        ...
        input_mode=np.asarray([INPUT_MODE_CONTEXT_COMMAND_V1]),
    )
...
def test_linear_behavior_clone_policy_command_context_mode_uses_command_features(tmp_path: Path) -> None:
    model = tmp_path / "linear_behavior_clone_context.npz"
    _write_command_context_linear_model(model)
    builder = ObservationBuilder(ObservationBuilderConfig(joint_names=JOINTS))
    policy = LinearBehaviorClonePolicy(model, observation_builder=builder)
    ...
    assert policy.input_mode == INPUT_MODE_CONTEXT_COMMAND_V1
    assert policy.get_observation() is not None
