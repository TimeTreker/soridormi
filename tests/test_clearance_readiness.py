...
def test_readiness_blocks_low_clearance_with_manifest_thresholds(tmp_path: Path) -> None:
    ...
    report = build_clearance_readiness(profile="candidate", suite_dir=suite_dir)
...
def test_readiness_compares_candidate_against_retained_reference(tmp_path: Path) -> None:
    ...
    report = build_clearance_readiness(
        profile="candidate",
...
def test_readiness_can_require_reference_improvement_from_cli(tmp_path: Path) -> None:
    ...
    result = subprocess.run(
        [
            sys.executable,
            "-m",
            "soridormi_runtime.clearance_readiness",
            "--profile-name",
            "candidate",
            "--suite-dir",
            str(candidate_suite),
...
def test_clearance_readiness_cli_functionally_writes_reports(tmp_path: Path) -> None:
...
def test_clearance_readiness_script_functionally_preserves_wrapper(tmp_path: Path) -> None:
