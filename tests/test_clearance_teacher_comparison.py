...
from soridormi_runtime.clearance_teacher_comparison import compare_clearance_teacher_suites
...
def test_teacher_comparison_passes_matching_candidate_without_claiming_clearance() -> None:
    result = compare_clearance_teacher_suites(_suite(), _suite(distance_scale=0.9))
...
def test_teacher_comparison_fails_slow_candidate() -> None:
    result = compare_clearance_teacher_suites(_suite(), _suite(distance_scale=0.5))
...
def test_teacher_comparison_fails_missing_required_scenario() -> None:
    ...
    result = compare_clearance_teacher_suites(_suite(), candidate)
