...
from soridormi_runtime.train_behavior_clone import INPUT_MODE_CONTEXT_COMMAND_V1
...
def test_train_neural_behavior_clone_accepts_command_context_mode_checkpoint_only(tmp_path: Path) -> None:
...
def test_train_neural_behavior_clone_exports_command_context_onnx_profile(tmp_path: Path, monkeypatch) -> None:
    prepared = _context_prepared_dataset(tmp_path)
    ...
    result = train_neural_behavior_clone(
        ...
        export_onnx=True,
        create_profile=True,
        profile_name="context_command_test",
    ...
    assert profile["model"]["input_mode"] == INPUT_MODE_CONTEXT_COMMAND_V1
    assert profile["contract"]["policy_input_size"] == 104
    assert profile["contract"]["input_mode"] == INPUT_MODE_CONTEXT_COMMAND_V1
