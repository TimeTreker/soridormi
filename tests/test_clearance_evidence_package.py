...
from soridormi_runtime.clearance_evidence_package import (
    build_clearance_evidence_package,
...
def test_evidence_package_ready_for_visual_review_when_quantitative_and_plan_pass(tmp_path: Path) -> None:
    ...
    package = build_clearance_evidence_package(
        profile="candidate",
...
def test_clearance_evidence_package_cli_functionally_writes_manifest_and_templates(tmp_path: Path) -> None:
    readiness_path = tmp_path / "readiness" / "clearance_readiness.json"
    visual_plan_path = tmp_path / "visual" / "policy_visual_inspection_plan.json"
    output_dir = tmp_path / "evidence"
    _write_json(readiness_path, _readiness_payload(tmp_path, ok=True))
    ...
    result = subprocess.run(
        [
            sys.executable,
            "-m",
            "soridormi_runtime.clearance_evidence_package",
            "--profile-name",
            "candidate",
            "--output-dir",
            str(output_dir),
...
def test_clearance_evidence_package_script_functionally_blocks_missing_readiness(tmp_path: Path) -> None:
    output_dir = tmp_path / "evidence"
