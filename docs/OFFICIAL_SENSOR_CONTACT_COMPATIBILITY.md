# Official sensor/contact compatibility

official reference-data mounting aligned Soridormi's imitation phase with the official Open Duck trace. The
next largest trace mismatches were IMU observations and foot contacts.

sensor and contact compatibility keeps the ONNX policy path unchanged, but makes the MuJoCo server closer to
