# Official gait-phase compatibility

The official trace comparison showed the largest Soridormi-vs-official mismatch in the ONNX observation was the imitation phase segment.
...
official phase compatibility changes Soridormi's phase generator to match that behavior:
...
The imitation phase mean error should drop compared with official trace comparison. If phase improves but the robot still does not step forward, the next mismatch to port is foot-contact timing/extraction.


## official reference-data mounting note: runtime reference-data mount

official phase compatibility introduced automatic Open Duck phase-period loading from
`polynomial_coefficients.pkl`, but the runtime container must be able to read
`/workspaces/Open_Duck_Playground`. official reference-data mounting mounts `./workspace/Open_Duck_Playground`
