# Official and Soridormi trace comparison

official baseline reproduction proved that the official Open Duck Mini v2 MuJoCo + ONNX inference loop walks forward in the same Docker/MuJoCo environment. official inference compatibility moved Soridormi closer to that loop, but the robot can still wiggle without useful forward displacement.

official trace comparison stops guessing by recording comparable per-policy-step traces from both systems.
