...
Please continue with M5: model replacement interface. Do not train a new model yet. M5.1 added static observation/action contract export, M5.2 made `check_policy_model.sh --profile NAME` validate both the static runtime contract and the actual ONNX IO metadata before runtime, M5.3 centralized ONNX Runtime provider selection so CUDA/CPU/TensorRT choices can be inspected and enforced by preflight checks, and M5.4 added a profile scaffolder for drop-in replacement ONNX profiles. Continue hardening compatible ONNX replacement workflows while preserving the same runtime/API/backend invariants.
```
