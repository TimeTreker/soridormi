...
Please continue with M5: model replacement interface. Do not train a new model yet. The target is to make compatible ONNX model replacement boring: export a static observation/action contract, validate profile/model compatibility, document required metadata, and keep the same runtime/API/backend invariants.
```
