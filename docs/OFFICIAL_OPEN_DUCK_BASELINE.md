# Official Open Duck baseline reproduction

official baseline reproduction changes the debugging strategy: before tuning Soridormi's ONNX wrapper any further, run the official Open Duck Mini v2 MuJoCo inference path inside the same Docker stack.
...
official baseline reproduction runs the official `playground.open_duck_mini_v2.mujoco_infer.MjInfer` class and only wraps it with a fixed non-interactive command and finite runtime.
...
## Why official baseline reproduction uses a tiny wrapper

The official script is interactive and infinite. official baseline reproduction reuses the official class but adds:
...
training environment and imports JAX-related packages. Soridormi's official baseline runner
...
Open Duck training dependencies in the policy-training image instead of the lightweight sim
baseline image.

## JAX-free asset loader note

The official `base.py` imports JAX/MJX training dependencies, but the official baseline reproduction
