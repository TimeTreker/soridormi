...
replaceable policy interface starts after the official Open Duck policy can be reproduced through the
...
## profile and model preflight profile/model preflight gate
...
## ONNX execution provider selection ONNX execution providers
...
## policy profile suite validation profile-suite validation
...
contract checks from model replacement contract without loading ONNX models:
...
## static policy checks CI static-check gate
...
the policy profile scaffolding scaffolder, validates that generated profile, and runs the replaceable policy interface unit test
set.
...
## policy manifest export replacement manifest export
...
## policy package verification replacement package and verifier

Use the package workflow when a replacement profile is ready to move between
machines, releases, or review sessions. It wraps the policy acceptance gate acceptance artifacts
...
## policy package install and restore policy package install/restore

Adds `./scripts/install_policy_package.sh PACKAGE.policy.tar.gz`, the inverse of the policy package verification package command. It verifies the tarball, restores `profile.yaml` into `configs/policies/`, optionally copies embedded ONNX model bytes into `data/policy_models/<profile>/`, and rewrites `model.path` to the runtime-visible `/data/policy_models/...` location. Use `--force` to overwrite an existing installed profile/model.
...
Accepted and packaged replacement models are still runtime artifacts. training and evaluation backbone begins converting runtime experience into training artifacts. Use:
...
## training and evaluation backbone dataset preparation handoff

After accepting and packaging a replacement profile, policy-training data can be exported from runtime logs with `./scripts/export_training_dataset.sh` and prepared with `./scripts/prepare_training_dataset.sh`. The preparation step validates the 101D observation and 14D action vectors, checks finite numeric values, and writes deterministic train/val/test split files plus `prepared_manifest.json`.

## linear behavior-cloning baseline behavior cloning baseline trainer

Use `./scripts/train_behavior_clone.sh PREPARED_DATASET` after dataset preparation and summarization. The trainer fits a deterministic linear ridge-regression behavior-cloning baseline from normalized observations to normalized actions, then saves a NumPy `.npz` artifact plus metrics/report files. This gives training and evaluation backbone a reproducible offline learning smoke test before adding heavier neural training or ONNX export.

## linear behavior-clone runtime profile linear behavior-clone baseline profiles

linear behavior-cloning baseline writes a deterministic NumPy baseline at `linear_behavior_clone.npz`. linear behavior-clone runtime profile adds a temporary runtime-compatible profile path for that artifact:
...
## candidate leaderboard policy candidate leaderboard

After one or more policy-training pipelines have produced evaluation artifacts, rank candidate profiles offline:
...
## candidate promotion policy candidate promotion
